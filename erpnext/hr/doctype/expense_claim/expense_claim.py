...
class ExpenseClaim(AccountsController):
    ...
    def after_insert(self):
        frappe.db.set_value("Travel Management", self.travel_management , "expense_claim", self.name)
        frappe.db.commit()
    ...
    def calculate_total_amount(self):
        ...
        self.total_claimed_amount = self.claim_amount + self.claim_amount1 + self.claim_amount_h + self.claim_amount_m + self.claim_amount_c + self.claim_amount_o
        self.total_sanctioned_amount = self.sanctioned_amount + self.sanctioned_amount1 + self.sanctioned_amount_h + self.sanctioned_amount_m + self.sanctioned_amount_c + self.sanctioned_amount_o
        if self.approval_status == 'Rejected':
            self.total_sanctioned_amount = 0.0
        # for d in self.get('expenses'):
        #     if self.approval_status == 'Rejected':
        #         d.sanctioned_amount = 0.0
        #     frappe.errprint(d.claim_amount)
    ...
    # def update_tvm(self):
    #     if self.travel_management_id:
    #         tvm = frappe.get_doc("Travel Management",self.travel_management_id)
    #         if not tvm.expense_claim:
    #             tvm.update({
    #                 "expense_claim":self.name
    #             })
    #             tvm.save()
...
@frappe.whitelist()
def update_travel(tr_name,name):
    frappe.errprint(name)
    frappe.db.set_value("Travel Management", tr_name , "expense_claim_status", name)
    frappe.db.commit()
    return "OK"
